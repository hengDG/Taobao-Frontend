...
const ExploreProduct = ({ refreshKey = 0 }: ExploreProductProps) => {
  ...
  const processTranslationQueue = useCallback(() => {
    while (
      ...
      void productService
        .translateTexts([source], language, "title", controller.signal)
        .then((response) => {
          if (controller.signal.aborted) {
            return;
          }
          ...
          if (!translatedText) {
            console.log("No translation returned:", source);
        ...
        .catch((error: any) => {
          ...
          console.error("Translate title error:", source, error);
        })
        .finally(() => {
          translationControllerRefs.current =
  ...
  const fetchProducts = useCallback(
    async (nextCursor?: string | null) => {
      ...
      try {
        ...
        setProducts((previousProducts) =>
          isLoadMore ? [...previousProducts, ...newProducts] : newProducts,
        );
        ...
        if (
          ...
          console.log("Explore request canceled");
  ...
  useEffect(() => {
    ...
    void fetchProducts();
  }, [abortPendingTranslations, fetchProducts, refreshKey]);
