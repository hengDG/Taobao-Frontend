  ...
  const handleSelect = () => {
    ...
    if (productId) {
      navigate(`/products/${productId}`, {
        ...
        // state: {
        //   initialPrice: usdPriceText || product.priceText || null,
        // },
      });
    }
    ...
    >
      <div className="relative aspect-[4/4] w-full overflow-hidden bg-white">
        ...
        >
          ...
          <span className="max-w-0 overflow-hidden whitespace-nowrap text-[10px] font-semibold text-[#194891] opacity-0 transition-all duration-200 group-hover/search:max-w-[84px] group-hover/search:opacity-100 group-hover/search:pr-2.5">
            Search
          </span>
        </button>
      </div>
      ...
      <div className="space-y-0 p-3 sm:p-2">
        <h3 className="line-clamp-2 text-[13px] leading-5 font-semibold text-slate-800">
          <img
            src="/taobao icon.png"
            alt="Taobao"
                ...
                <div key={index} className="h-5">
                  {item.type === "original" ? (
                    <div className="text-[11px] text-slate-400">
                      Before:{" "}
                      <span className="text-[12px] font-bold line-through text-[#c9171d]">
                        {item.content}
                      </span>
                    </div>
                  ) : (
                    <div className="flex h-5 items-center text-[11px] font-semibold text-[#c68d06]">
                      {item.content} - China
                    </div>
                  )}
                </div>
