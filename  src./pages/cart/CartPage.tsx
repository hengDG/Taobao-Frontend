...
export function CartPage({
  ...
  const groupedItems = useMemo(() => {
    const groups = new Map<
      string,
      { shopName: string; items: CartLineItem[] }
    >();
    ...
    items.forEach((item) => {
      ...
      groups.set(shopName, { shopName, items: [item] });
    });
      ...
      <main className="mx-auto w-full max-w-5xl px-4 py-10 sm:px-6">
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center">
          ...
          <button
            ...
            className="mt-4 inline-flex rounded-xl bg-[#1f5fb8] px-4 py-2 text-sm font-semibold text-white transition hover:brightness-95"
          >
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 pb-28 pt-4 sm:px-6 lg:py-8">
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1 self-start pt-1">
                          <Link to={`/product/${item.product.id}`}>
                            <p className="line-clamp-2 text-[14px] font-medium leading-6 text-slate-800 transition hover:text-[#ff5000]">
                              {localizedTitle}
                            </p>
                          </Link>
                        ...
                        <div className="hidden pt-1 text-xs leading-5 text-slate-500 lg:block">
                          <p>
                            ...
                            <span className="ml-1 text-slate-600">Classic</span>
                          </p>
                          <p>
                            {language === "km" ? "ទំហំ" : "Model"}:
        ...
        <aside className="mt-5 lg:mt-0">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-24">
            ...
            <p className="mt-1 text-xs text-slate-500">
              {selectedCount} items · {selectedUnitCount} units
            </p>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xl font-bold text-[#1b2f4e]">
            ...
            <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3">
              ...
              <ul className="mt-1 space-y-1 text-xs text-amber-700">
                <li>Select items from the same shop to reduce shipping cost</li>
                <li>
            ...
            <div className="mt-4 flex items-center justify-center gap-2">
              <button
                type="button"
                onClick={() => setAllSelected(!allSelected)}
                className="w-[35%] rounded-lg border border-slate-300 py-3 text-xs font-semibold text-slate-700 transition hover:border-[#1f5fb8] hover:text-[#1f5fb8]"
              >
                {allSelected ? unselectAllText : selectAllText}
              </button>
              <button
                ...
                disabled={selectedCount === 0}
                onClick={onCheckout}
                className="w-[65%] rounded-lg bg-[#ff5000] px-5 py-3 text-sm font-semibold text-white transition hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {checkoutText}
              </button>
            </div>
      ...
      {deleteSingleItemId ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 p-4">
          <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-4 shadow-xl">
            <p className="text-sm font-semibold text-slate-800">
              {language === "km" ? "លុបទំនិញនេះหรือ?" : "Delete this item?"}
            </p>
            <p className="mt-1 text-xs leading-5 text-slate-500">
              {language === "km"
