...
export function HomeMarketplacePanel() {
  ...
  return (
    <section className="relative mx-auto mt-3 grid w-full max-w-[95%] gap-4 pb-6 lg:grid-cols-12">
      <article
        className={`relative h-37.5 overflow-hidden rounded-3xl p-4 text-white shadow-[0_14px_28px_rgba(34,74,130,0.2)] sm:min-h-67.5 sm:p-5 lg:col-span-2 lg:h-full ${activeSlide.bgClass}`}
      >
      ...
      <article className="rounded-3xl border border-[#e5ecff] bg-[#f7f9ff] p-4 sm:p-5 lg:col-span-7 2xl:col-span-7">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h3 className="text-lg font-extrabold text-[#263247] sm:text-lg 2xl:text-xl">
          ...
          >
            Show more <span className="block lg:hidden">products</span>
          </a>
        </div>
          ...
          {products.map((product, index) => (
            <div
              ...
              className={`rounded-2xl border border-[#edf2ff] bg-white p-2.5 shadow-[0_10px_24px_rgba(19,40,96,0.07)] sm:p-3 ${
                index >= 5 ? "hidden" : index >= 4 ? "hidden 2xl:block" : ""
              }`}
            >
              {product.imageUrl ? (
                <img
              ...
              <p className="mt-2 line-clamp-2 text-xs font-semibold leading-4 text-[#2a3044] sm:text-sm sm:leading-5">
      ...
      <article className="relative w-full overflow-hidden rounded-3xl border border-[#d9e4ff] bg-gradient-to-b from-white to-[#edf2ff] p-4 shadow-[0_14px_32px_rgba(26,66,152,0.16)] sm:p-5 lg:col-span-3 2xl:col-span-3 lg:justify-self-end">
